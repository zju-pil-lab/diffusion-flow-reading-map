...
QUERY = 'all:"diffusion model" OR all:"score-based generative model" OR all:"flow matching" OR all:"rectified flow"'


def known_arxiv_ids() -> set[str]:
    papers = json.loads((ROOT / "data" / "papers.json").read_text(encoding="utf-8"))
    return {paper["arxivId"] for paper in papers if paper.get("arxivId")}
...
def fetch_candidates(days: int, limit: int) -> list[dict]:
    ...
    request = urllib.request.Request(
        f"{ARXIV_API}?{params}",
        headers={"User-Agent": "Diffusion-Flow-Reading-Map/1.0 (paper discovery; GitHub Actions)"},
    )
    ...
    for entry in root.findall("atom:entry", ATOM):
        ...
        if not any(term in haystack for term in ("diffusion", "flow matching", "rectified flow", "score-based generative")):
            continue
